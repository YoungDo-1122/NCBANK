...
public interface GroupAccountMapper {
	...
	@Select("SELECT a.auto_type, a.auto_next_date, a.auto_money, ac.ac_password " + "FROM auto a, account ac, member m "
			+ "WHERE ac.user_num = m.user_num AND ac.account = #{account} AND m.user_num = #{user_num}")
	@Results({ @Result(property = "auto_type", column = "auto_type"),
	...
	@Select("SELECT a.account, g.group_num, g.user_num, g.group_leader " + "FROM groupinfo g, account a "
	...
	@Select("SELECT a.auto_money, a.auto_type, a.auto_next_date, a.auto_end " + "FROM auto a, account ac "
			+ "where ac.account = #{account} and ac.ac_type = 1")
	AutoBean getAutoInfo(@Param("account") String account);

	@Select("SELECT g.group_joindate, g.group_leader, g.group_name " + "FROM groupinfo g "
			+ "JOIN account acc ON g.user_num = acc.user_num " + "WHERE acc.account = #{account} AND acc.ac_type = 1")
	GroupAccountBean getGroupInfobyAcc(@Param("account") String account);

	@Select("SELECT m.name, m.phone " + "FROM member m " + "JOIN groupinfo g ON g.user_num = m.user_num "
	...
	@Select("SELECT g.group_leader, g.group_joindate " + "FROM groupinfo g "
			+ "WHERE g.group_num = (SELECT group_num FROM groupinfo WHERE user_num = #{user_num})")
	List<GroupAccountBean> getGroupMemberDetails(@Param("user_num") int user_num);

	@Select("SELECT m.name " + "FROM member m, groupinfo g "
	...
	@Select("select a.ac_balance " + "from account a, groupinfo g "
